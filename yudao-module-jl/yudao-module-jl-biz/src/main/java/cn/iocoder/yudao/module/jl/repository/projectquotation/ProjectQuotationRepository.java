...
public interface ProjectQuotationRepository extends JpaRepository<ProjectQuotation, Long>, JpaSpecificationExecutor<ProjectQuotation> {
    @Transactional
    @Modifying
    @Query("update ProjectQuotation p set p.originPrice = ?1, p.resultPrice = ?2 where p.id = ?3")
    int updateOriginPriceAndResultPriceById(BigDecimal originPrice, BigDecimal resultPrice, Long id);
    @Transactional
    @Modifying
